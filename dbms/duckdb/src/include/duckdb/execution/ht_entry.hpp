...
public:
#if defined(DUCKDB_DISABLE_POINTER_SALT)
	//! No salt, all pointer
	static constexpr const hash_t SALT_MASK = 0x0000000000000000;
	static constexpr const hash_t POINTER_MASK = 0xFFFFFFFFFFFFFFFF;
#elif defined(MTE)
	//! Upper 16 bits minus MTE bits are salt, lower 48 bits are the pointer
	static constexpr const hash_t SALT_MASK = 0xF0FF000000000000;
	static constexpr const hash_t POINTER_MASK = 0x0F00FFFFFFFFFFFF;
#else
