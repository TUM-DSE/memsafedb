...
#ifdef MTE
	size_t mte_usize = 0;
	if (alloc_ctx != NULL && alloc_ctx->szind != SC_NSIZES) {
		mte_usize = sz_index2size(alloc_ctx->szind);
	} else if (!tsdn_null(tsdn)) {
		emap_alloc_ctx_t mte_ctx;
		emap_alloc_ctx_lookup(tsdn, &arena_emap_global, ptr, &mte_ctx);
		if (mte_ctx.szind != SC_NSIZES) {
			mte_usize = sz_index2size(mte_ctx.szind);
		}
	}
	if (mte_usize > 0) {
		ptr = untag_memory_region(ptr, mte_usize);
	}
#endif
	assert(!is_internal || tcache == NULL);
...
#ifdef MTE
	if (likely(size > 0)) {
		ptr = untag_memory_region(ptr, size);
	}
#endif
