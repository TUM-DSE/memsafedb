...
MEM_ROOT::Block *MEM_ROOT::AllocBlock(size_t wanted_length,
  ...
  size_t length = std::max(wanted_length, mem_root_align_size(minimum_length));
  if (m_max_capacity != 0) {
    size_t bytes_left;
  ...
  const size_t bytes_to_alloc = length + mem_root_align_size(sizeof(Block));
...
void *MEM_ROOT::AllocSlow(size_t length) {
  ...
  if (length >= m_block_size || MEM_ROOT_SINGLE_CHUNKS) {
    ...
    void *ret = pointer_cast<char *>(new_block) +
                mem_root_align_size(sizeof(*new_block));
    return mem_root_tag_memory(ret, length);
  }
  ...
  return mem_root_tag_memory(new_mem, length);
}
...
bool MEM_ROOT::ForceNewBlock(size_t minimum_length) {
  ...
  Block *new_block = AllocBlock(/*wanted_length=*/mem_root_align_size(m_block_size),
  ...
  char *new_mem = pointer_cast<char *>(new_block) +
                  mem_root_align_size(sizeof(*new_block));
...
void MEM_ROOT::ClearForReuse() {
  ...
  m_current_free_start = pointer_cast<char *>(m_current_block) +
                         mem_root_align_size(sizeof(*m_current_block));
