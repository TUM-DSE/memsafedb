...
#if defined(MTE) && defined(PROT_MTE)
  prot |= PROT_MTE;
#endif
  void *ptr = mmap(nullptr, n_bytes, prot, MAP_PRIVATE | MAP_ANON, -1, 0);
...
inline bool page_aligned_free(void *ptr, size_t n_bytes [[maybe_unused]]) {
  ...
  void *untagged = ptr;
#ifdef MTE
  untagged = clear_tag(ptr);
#endif
  auto ret = munmap(untagged, n_bytes);
  if (unlikely(ret != 0)) {
    ib::log_error(ER_IB_MSG_858)
...
#ifdef MTE
    mem = tag_memory_region(mem, total_len);
#endif
