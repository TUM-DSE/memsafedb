...
class Block : private Header {
 public:
   ...
   * this value. */
#ifdef MTE
  static constexpr size_t ALIGN_TO = 16;  // MTE tag granule size
#else
...
inline Chunk Block::allocate(size_t chunk_size) noexcept {
  ...
  const size_t total_chunk_size = Chunk::size_hint(chunk_size_aligned);

  /* Remove the "no access" flag we set on this memory during block
   * creation. Relax it to report read+depend_on_contents. */
  MEM_UNDEFINED(Header::next_available_slot(), total_chunk_size);

  uint8_t *chunk_start = Header::next_available_slot();
#ifdef MTE
  /* Tag the chunk memory region before creating the Chunk object.
   * This ensures the chunk metadata and user data have a unique tag,
   * providing isolation from adjacent chunks. */
  chunk_start = reinterpret_cast<uint8_t *>(
      tag_memory_region(chunk_start, round_to_tag_granule(total_chunk_size)));
#endif

  Chunk chunk{chunk_start, Header::first_pristine_offset()};
  Header::increment_number_of_used_chunks(total_chunk_size);
