...
#ifdef MTE
  static constexpr size_t METADATA_SIZE = round_to_tag_granule(sizeof(Chunk::metadata_type));
#else
...
inline uint8_t *Chunk::block() const {
  uint8_t *block_ptr = m_offset - offset();
#ifdef MTE
  // Load the correct tag from the block header memory
  return reinterpret_cast<uint8_t *>(load_tag(block_ptr));
#else
  return block_ptr;
#endif
}
