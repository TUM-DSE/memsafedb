...
static inline size_t mem_root_align_size(size_t length) {
#ifdef MTE
  return round_to_tag_granule(length);
#else
  return ALIGN_SIZE(length);
#endif
}

static inline void *mem_root_tag_memory(void *ptr, size_t length) {
#ifdef MTE
  return tag_memory_region(ptr, length);
#else
  return ptr;
#endif
}
...
struct MEM_ROOT {
 ...
 public:
   ...
   *
   * The returned pointer will always be 8-aligned (16-aligned with MTE).
  */
  void *Alloc(size_t length) MY_ATTRIBUTE((malloc)) {
    length = mem_root_align_size(length);
