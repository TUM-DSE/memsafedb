...
#ifdef MTE
    void tagFrame(common::frame_idx_t frameIdx) const;
#endif
...
#ifdef MTE
        return static_cast<uint8_t*>(__arm_mte_get_tag(this->getRawFrame(frameIdx)));
#else
        return this->getRawFrame(frameIdx);
#endif
    }

private:
    uint8_t *getRawFrame(common::frame_idx_t frameIdx) const {
