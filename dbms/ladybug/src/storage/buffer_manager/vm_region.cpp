...
VMRegion::VMRegion(PageSizeClass pageSizeClass, uint64_t maxRegionSize) : numFrameGroups{0} {
    ...
    prot |= PROT_MTE;
#endif
    region = static_cast<uint8_t*>(mmap(NULL, getMaxRegionSize(), prot,
...
#ifdef MTE
void VMRegion::tagFrame(frame_idx_t frameIdx) const {
    auto* frame = clear_tag(getRawFrame(frameIdx));
    auto size = round_to_tag_granule(frameSize);
    tag_memory_region(frame, size);
}
#endif
...
void VMRegion::releaseFrame(frame_idx_t frameIdx) const {
    ...
    auto* frame = clear_tag(getRawFrame(frameIdx));
    auto size = round_to_tag_granule(frameSize);
    untag_memory_region(frame, size);
#endif
