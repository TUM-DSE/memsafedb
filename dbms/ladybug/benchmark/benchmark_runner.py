...
datasets = {'ldbc-sf01', 'ldbc-sf1', 'ldbc-sf10', 'ldbc-sf100', 'click', 'graph500-27', 'soc-livejournal', 'datagen-sf10k'}
datasets_benchmark_severs_url_key = {
    'ldbc-sf01': 'BENCHMARK_SERVER_URL',
    'ldbc-sf1': 'BENCHMARK_SERVER_URL',
...
lbug_benchmark_tool = os.path.join(
    os.path.dirname(serialized_base_dir), 'release', 'tools', 'benchmark', 'lbug_benchmark')
...
serialized_graphs_path = {
    'ldbc-sf01-ku': os.path.join(serialized_base_dir, 'ldbc-sf01-serialized'),
    'ldbc-sf1-ku': os.path.join(serialized_base_dir, 'ldbc-sf1-serialized'),
...
def serialize_dataset(dataset_name, lbug_shell_tool):
    ...
    try:
        subprocess.run([sys.executable, serializer_script, dataset_name,
                        dataset_path, serialized_graph_path, benchmark_copy_log_dir, "--lbug-shell", lbug_shell_tool], check=True)
...
if __name__ == '__main__':
    ...
    lbug_shell_tool = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(lbug_benchmark_tool))), 
        'tools',
        'shell', 
        'lbug'
    )
    logging.info("Using lbug shell tool: %s", lbug_shell_tool)

    # serialize dataset
    serialize_dataset(args.dataset + '-ku', lbug_shell_tool)
