...
#else
/* CHERI requires 16-byte alignment for capabilities */
#ifdef __CHERI_PURE_CAPABILITY__
...
static inline void *ztrymalloc_usable_internal(size_t size, size_t *usable) {
    ...
    *((size_t*)ptr) = size;
    ...
    if (usable) *usable = size;
#ifdef __CHERI_PURE_CAPABILITY__
    /* On CHERI, set bounds to cover the full allocation (including prefix)
     * so that zmalloc_size can read the size header backwards */
    void *ret = cheri_bounds_set(ptr, size + PREFIX_SIZE);
    ret = (char*)ret + PREFIX_SIZE;
    return ret;
#else
...
static inline void *ztryrealloc_usable_internal(void *ptr, size_t size, size_t *usable, size_t *old_usable) {
    ...
    *old_usable = oldsize;
    return newptr;
#else
    /* On CHERI, ptr already has bounds covering the full allocation (including prefix)
     * so we can safely access backwards to read the size header */
    ...
    *old_usable = oldsize;
    ...
    /* On CHERI, set bounds to cover the full allocation (including prefix)
     * so that zmalloc_size can read the size header backwards */
    void *ret = cheri_bounds_set(newptr, size + PREFIX_SIZE);
    ret = (char*)ret + PREFIX_SIZE;
    return ret;
#else
...
size_t zmalloc_size(void *ptr) {
    /* On CHERI, ptr has bounds covering the full allocation (including prefix)
     * so we can safely access backwards to read the size header */
