...
/*
 * If we're on CHERI purecap, we need to double the size of the keylength.
 * We have:
 * [8byte be expire time]+[8 byte padding]+[16 byte client ID (capability)]
 */ 

#ifdef __CHERI_PURE_CAPABILITY__
#define CLIENT_ST_KEYLEN 32    /* see above */
#else
...
void decodeTimeoutKey(unsigned char *buf, uint64_t *toptr, client **cptr) {
    ...
    *toptr = ntohu64(*toptr);
#ifdef __CHERI_PURE_CAPABILITY__
    memcpy(cptr,buf+16,sizeof(*cptr));
#else
