...
#ifdef MTE
	size = round_to_tag_granule(size);
	size_t mte_usize = 0;
	size_t *usizep = (usize != NULL) ? usize : &mte_usize;
	void *ret = imalloc_fastpath(size, &malloc_default, usizep);
	if (ret != NULL) {
		size_t tag_size = (*usizep != 0) ? *usizep : sz_s2u(size);
		if (tag_size == 0) {
			tag_size = size;
		}
		ret = tag_memory_region(ret, tag_size);
	}
	return ret;
#else
...
je_posix_memalign(void **memptr, size_t alignment, size_t size) {
...
#ifdef MTE
	size = round_to_tag_granule(size);
	if (alignment < MTE_GRANULE_SIZE) {
		alignment = MTE_GRANULE_SIZE;
	}
#endif
	...
	if (ret == 0 && *memptr != NULL) {
		size_t tag_size = dopts.usize != 0 ? dopts.usize : size;
		*memptr = tag_memory_region(*memptr, tag_size);
	}
#endif
...
#ifdef MTE
	if (ret != NULL) {
		size_t tag_size = dopts.usize != 0 ? dopts.usize : num * size;
		ret = tag_and_zero_memory_region(ret, tag_size);
	}
#endif
...
#ifdef MTE
	/* Probe for UAF: a freed allocation will be re-tagged. */
	if (likely(usize > 0)) {
		((unsigned char *)ptr)[0] = 0;
	}
	ptr = untag_memory_region(ptr, usize);
#endif
	if (config_prof && opt_prof) {
...
bool free_fastpath(void *ptr, size_t size, bool size_hint, size_t *usable_size) {
	...
	void *untagged_ptr = ptr;
#ifdef MTE
	untagged_ptr = clear_tag(ptr);
#endif
	...
	if (!cache_bin_dalloc_easy(bin, untagged_ptr)) {
		return false;
	}

#ifdef MTE
	if (likely(usize > 0)) {
		/* Probe to check the memory has not been freed by somebody else. */
		((unsigned char *)ptr)[0] = 0;
		(void)untag_memory_region(ptr, usize);
	}
#endif
...
#ifdef MTE
	if (ret != NULL) {
		size_t tag_size = dopts.usize != 0 ? dopts.usize : dopts.item_size;
		ret = tag_memory_region(ret, tag_size);
	}
#endif
...
#ifdef MTE
	if (ret.ptr != NULL) {
		ret.ptr = dopts.zero ? tag_and_zero_memory_region(ret.ptr,
		    ret.size) : tag_memory_region(ret.ptr, ret.size);
	}
#endif
...
je_mallocx(size_t size, int flags) {
	...
	if (ret != NULL) {
		size_t tag_size = (dopts.usize != 0) ? dopts.usize : size;
		ret = dopts.zero ? tag_and_zero_memory_region(ret, tag_size)
				 : tag_memory_region(ret, tag_size);
	}
#endif
