...
class SkipList {
 ...
 private:
  ...
  int RandomHeight();
  bool Equal(const char* a, const char* b) const { return (strcmp(a, b) == 0); }
...
inline const char* SkipList::Iterator::key() const {
  assert(Valid());
  return node_->key;
}
...
inline void SkipList::Iterator::Seek(const char* target) {
...
bool SkipList::KeyIsAfterNode(const char* key, Node* n) const {
  // null n is considered infinite
  return (n != nullptr) && (strcmp(n->key, key) < 0);
}

typename SkipList::Node*
SkipList::FindGreaterOrEqual(const char* key,
                                              Node** prev) const {
  Node* x = head_;
...
typename SkipList::Node*
SkipList::FindLessThan(const char* key) const {
  Node* x = head_;
  int level = GetMaxHeight() - 1;
  while (true) {
    assert(x == head_ || strcmp(x->key, key) < 0);
    Node* next = x->Next(level);
    if (next == nullptr || strcmp(next->key, key) >= 0) {
      if (level == 0) {
        return x;
      } else {
        // Switch to next list
...
void SkipList::Insert(const char* key) {
...
bool SkipList::Contains(const char* key) const {
