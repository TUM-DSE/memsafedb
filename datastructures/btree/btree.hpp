...
#define die(msg) do { perror(msg); exit(EXIT_FAILURE); } while(0)
  ...
  struct BufferManager {
    ...
    PID toPID(void* page) { 
      ...
      return reinterpret_cast<Page*>(addr) - virtMem;  
    #else
  ...
  void BufferManager::init(u64 memsize){
    virtSize = memsize;
    physSize = memsize;
    ...
    flags |= PROT_MTE;
    #endif
    virtMem = (Page*)mmap(NULL, virtAllocSize, flags, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
  ...
  Page* BufferManager::allocPage() {
    ...
    assert(getPageState(pid).tryLockX(stateAndVersion));
  ...
  struct BTreeNodeHeader {
    ...
    union {
      ...
      PID nextLeafNode = noNeighbour; // leaf = noNeighbour;
    };
    ...
    BTree::BTree() : splitOrdered(false), initialized(false){}
    void BTree::init(u64 memsize){
      initialized = true;
      bm.init(memsize);
