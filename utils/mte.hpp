...
inline void init_process(int mode){
  if (prctl(PR_SET_TAGGED_ADDR_CTRL, (1UL << 0) | PR_MTE_TCF_SYNC | (0xfffe << 3), 0, 0, 0)){
    perror("prctl() failed");
  }
}
...
inline void* tag_pointer(void* ptr, size_t size){
  ...
  assert(size % 16 == 0);
  void *t_ptr;
  __asm__ volatile("irg %0, %1" : "=r" (t_ptr): "r" (ptr));
  for (size_t i = 0; i < size; i += 16) {
    void *c_ptr = (void *)((uintptr_t)ptr + i);
    __asm__ volatile("stg %0, [%1]" : : "r" (t_ptr), "r" (c_ptr) : "memory");
  }
  return t_ptr;
}
...
inline void* untag_pointer(void* ptr, size_t size){
  ...
  for (size_t i = 0; i < size; i += 16) {
    void *c_ptr = (void *)((uintptr_t)ptr + i);
    __asm__ volatile("stg %0, [%1]" : : "r" (ut_ptr), "r" (c_ptr) : "memory");
  } 
  return ut_ptr;
}
